...
var CROSS_ORIGIN_ISOLATION_ERROR = [
...
async function compileRspackBrowserProject(input, options = {}, rspackBrowserModule) {
	...
	const outputText = replaceExternalImportSpecifiers(typeof output === "string" ? output : new TextDecoder().decode(output), getRspackDependencies(input.depsInfo));
	return {
		outputFileName,
...
function createProjectFiles(files, sourceAttributeNames, depsInfo) {
	const projectFiles = { "/package.json": JSON.stringify({ type: "module" }) };
	for (const [fileName, content] of Object.entries(files)) projectFiles[toProjectPath(fileName)] = isJSXSourceFile(fileName) ? require_constant.injectJSXSourceInfoAndCssImports(content, {
		filename: fileName,
		files,
		depsInfo,
...
function getRspackDependencies(depsInfo) {
	...
	for (const dependencyName of Object.keys(dependencies)) {
		const { packageName, subPath } = require_constant.parsePackagePath(dependencyName);
		if (!subPath || !depsInfo[packageName] || depsInfo[dependencyName]) continue;
...
function shouldExternalizeRequest(request, dependencies, externalRequests) {
	return externalRequests.has(request) || !!require_constant.resolveDependencyUrl(request, dependencies, require_constant.TRANSFORM_OPTIONS);
}
function replaceExternalImportSpecifiers(outputText, dependencies) {
	const replaceSpecifier = (specifier) => require_constant.resolveDependencyUrl(specifier, dependencies, require_constant.TRANSFORM_OPTIONS) ?? specifier;
	return outputText.replace(/(\bfrom\s*["'])([^"']+)(["'])/g, (match, start, specifier, end) => {
...
async function loadRspackBrowserModule() {
	if (globalThis.crossOriginIsolated === false) throw new Error(CROSS_ORIGIN_ISOLATION_ERROR);
...
//# sourceMappingURL=rspackBrowser-DstwN0An.cjs.map